  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.search(new RegExp('^(?:|\/*|(\/+index.html?)?)$')) != -1) {
      ...
      if (path.search(new RegExp('^(?:|\/(index.html?)?)$')) != -1) {
        ...
        // 其他配置参数
        let varnamelist = ['udp', 'tfo', 'mp', 'sp', 'rp', 'hp', 'tp', 'dns', 'secret', 'list'];
        let varlist = [];
...
function gen_html(pre) {
  let t = `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>sub2clashmeta</title><style>body{font-family:Arial,sans-serif;margin:20px}table{border-collapse:collapse}th,td{border:1px solid#dddddd;text-align:left;padding:8px;width:140px}textarea{width:1000px;height:75px}th{background-color:#f2f2f2}button{padding:8px 16px;background-color:#4CAF50;color:white;border:none;cursor:pointer}button:hover{background-color:#3c8a3f}tr:nth-child(even){background-color:#f9f9f9}input[type=text]{width:200px}</style><script>function processText(){let js={};var a,b;a=document.getElementById('inputText').value;a=a.trim();if(!a){return}js.url=a.split('\\n').join('|');a=document.getElementById('client');try{js.target=a.value}catch(e){}b=["udp","tfo"];for(let c in b){try{a=document.getElementsByName(b[c]);for(let i=1;i<a.length;i++){if(a[i].checked){js[b[c]]=a[i].value;break}}}catch(e){}}a=document.getElementById('lm');if(a.checked){js['list']='true'}else{b=['mp','sp','hp','rp','tp'];for(let c in b){try{a=document.getElementById(b[c]);let i=parseInt(a.value,10);if(i>=0&&i<65536){js[b[c]]=i}}catch(e){}}try{a=document.getElementsByName('dns');for(let i=1;i<a.length;i++){if(a[i].checked){js['dns']=a[i].value;break}}}catch(e){}a=document.getElementById('secret');try{a=a.value.trim();if(/^[0-9a-zA-Z]+$/.test(a)){js.secret=a}}catch(e){}}a=new URLSearchParams(js).toString();document.getElementById('outputText').value='${pre}'+a}function f(){var d;let a=document.getElementById('lm');let b=['mp','sp','hp','rp','tp','secret'];if(a.checked){b.forEach(c=>{d=document.getElementById(c);d.setAttribute('disabled','disabled')});d=document.getElementsByName('dns');d.forEach(c=>{c.setAttribute('disabled','disabled')})}else{b.forEach(c=>{d=document.getElementById(c);d.removeAttribute('disabled')});d=document.getElementsByName('dns');d.forEach(c=>{c.removeAttribute('disabled')})}}</script></head><body><h3>订阅转换</h3><table><tr><td>订阅链接：</td><td><textarea id="inputText"placeholder="支持订阅链接/base64订阅内容/ss/ssr/vmess/trojan/hysteria/hysteria2/vless节点，多个链接每行一个或用|分隔"></textarea><br></td></tr><tr><td>客户端：</td><td><select id="client"><option value="clash">mihomo/clash</option></select></td></tr><tr><td>UDP代理：</td><td><label><input type="radio"name="udp"value="2"checked/>默认</label><label><input type="radio"name="udp"value="1"/>使能</label><label><input type="radio"name="udp"value="0"/>禁用</label></td></tr><tr><td>tcp fast open：</td><td><label><input type="radio"name="tfo"value="2"checked/>默认</label><label><input type="radio"name="tfo"value="1"/>使能</label><label><input type="radio"name="tfo"value="0"/>禁用</label></td></tr><tr><td>mix-port：</td><td><input type="text"id="mp"placeholder="0-65535(留空为默认值，0为禁用)"></td></tr><tr><td>socks-port：</td><td><input type="text"id="sp"placeholder="同上"></td></tr><tr><td>port：</td><td><input type="text"id="hp"placeholder="同上"></td></tr><tr><td>redir-port：</td><td><input type="text"id="rp"placeholder="同上"></td></tr><tr><td>tproxy-port：</td><td><input type="text"id="tp"placeholder="同上"></td></tr><tr><td>UI访问密钥：</td><td><input type="text"id="secret"placeholder="字母数字组合(留空为默认值)"></td></tr><tr><td>DNS配置：</td><td><label><input type="radio"name="dns"value="1"checked/>使能(默认)</label><label><input type="radio"name="dns"value="2"/>仅监听</label><label><input type="radio"name="dns"value="0"/>禁用</label></td></tr><tr><td>输出为Node List：</td><td><input type="checkbox"id="lm"onchange="f()"unchecked></td></tr><tr><td>定制订阅：</td><td><textarea id="outputText"readonly></textarea></td></tr></table><br><button onclick="processText()">生成订阅链接</button><br></body></html>`;
  return new Response(t, { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' } });
}


function gen_icon() {
  var A = function (a = 0) { return 'A'.repeat(a); };
  const b64data = "AAABAAEAICAAAAEAIACoEAAAFgAAACgAAAAgAAAAQAAAAAEAIAAAAAAAgBAAAMMOAADDDg" + A(60) + "Dg4dY7y8zK/8zLzv/MzM3/0M/Q/9DRyP+prLr/v8DH/9jY1//09PUE" + A(112) + "293hMGVnvf+kpsD/2trJ/8rLxv/KzMH/dnjB/1BN2f+Vmbj/0dDI/97e4Cs" + A(106) + "PP08wR8fNThW1fl/2Rotf94ecb/b2/M/2hqxP9nYuz/c2v8/2dnyv/f4Nf/6eruFra34HOXl92pi4rfwoyK4MKgoOKaztDmS" + A(75) + "sbLafp+b8v+Pjej/hH/v/4qE+P+Aefj/gXr4/3t28P9wau//a2Xw/4eKzv9vbd//Ylzq/3Js8f9zbfP/bmj0/2hi8f9uZe3/kqDbtOvw8BI" + A(63) + "Cjo92YqqX4/8fE+f/r6vv/7+75/8nI9P9/ee//eXTw/3Br7v9ya/P/XFvT/3Bq7v+opPv/nZnz/5CM8v+FgfD/enbw/21k7P9HmuH/c7Xc6ern7Rk" + A(53) + "6OrvGqGf45/Oyvv/9PX8/7O1/P+rqvn/7u74/7e28v9xa/D/dG7v/29o9P9qaNr/qazU/6yq6v+emvX/mZX0/46J8v+FgfP/fWzx/1yM3f9FtOj/ibbbwg" + A(15) + "LW1/3M" + A(31) + "Dn6e4ap6Xo/+Lf/f+1t/v/m539/4+O+v+YlPP/0dXz/3Zw8P90b+//bWby/3Bt4f/y9PT/7/Dy/66u3/+emvX/mZX0/4uJ7v97heb/YqTj/1Ky5v9SruX/wdDjXQAAAAAAAAAAmJj/qQ" + A(31) + "PDx8wupqOeQ0s/7/7Gz+/+kpvv/mZn5/5yY9P/MzvP/d3Lw/3Vw7v9xafb/Y2LU/9nc5P8AAAAA9PXzBK2s4padnvH/gb7k/3bE6f9rvun/YLPm/0uv6P+Ju+DCAAAAAAAAAADGxv9W" + A(37) + "K6u3YW8uPn/7e37//X0/v/s7Pv/9vj4/6+u8f92cO//eXPu/3Jr8P9VVc7/cnHS/+7w8RAAAAAA2dnoOaGi56qQy+7/hMbu/3i/6v9uu+j/V7Po/2iw4Pvt6/ES" + A(48) + "wMTXZIyL4P/Szvv//f38///////U1Pr/iIL3/395+P9wavf/amTy/2xo4/9jXPD/lpfWwgAAAAAAAAAAqMHfnpvV8/+Qyu3/g8Xs/3rA6v9nuur/Xa3f/97h6ys" + A(47) + "B/f9LaVlbO/4eC9/+Sjvn/hoTl/21p4/9oZtj/ZWTL/25uwP9oabr/a2bt/4iB+/9tad7/0tXjSQAAAADd5Owxocvm/5rU8/+PzvL/h8ju/3XA7P9crN//4eTsJ" + A(43) + "u73ea2Jc5/95cfz/VlXW/1latv96eqn/iIiv/5uYsP+kn6//p6Gw/6ehrf9qaqz/enfp/5eS/P9xc7D/o6K2qq+vx47U2eT/tMzid53G456Lxem+fMnz/2Gp2//y7/ML" + A(42) + "CXl9qugnv5/2Nh4v94eaf/ysbI/+Pg4v/p5uf/4d/k/9zZ4v/W097/09Dd/9zZ2/+MjLP/bW3H/3l5sf+8tcP/oJu3/5GRsf/KydhPAAAAANbZ5Dljp9j/ea3W3w" + A(47) + "JWT4LWAfer/e3yn/9/b3f/w7vX/3d3q/9fW4v/q6Oz/5+Xq/+rp7f/r6u7/8O/z/9/c4P+9vMj/3dzk/+Tj5v+vp7f/jY2u/+jq8BkAAAAAAAAAAMjQ4FPI0OFT" + A(48) + "qqzbjHZ2qf/Z1df/+Pb6/5mZqf9IRH//cG+s/9rb4f/t7O//6unt/+rp7//q6e7/7u7z//39+v/7+/j/qqXK/358sv+mqcOQ8vP3Bw" + A(68) + "DAwtBktrLD//Hw8v/h4ev/oqOr/7Kysv/q6e3/1tbi/+no7v/x8PL/+fjz//v69P/5+PT/5OPz/5OR5v9UUeH/TEvX/9fZ6DY" + A(74) + "J6duaO+uMn/4+Ho/+np7f+rrNn/kJDR/97d5P/+/fj/+fj0//b19//S0fH/w8L1/7e29v93deX/YF3g/2Ng4P9RTd3/k5bPsPX39AQ" + A(63) + "Dj5e0hko+svLeyxP/6+fj/ycre/yoroP8AAJ3/bW60/+rq+P/T0vb/urnu/7+/8f+Ghsz/IiGf/1hesP9xcOH/ZGHg/1tY4v9ERMj/amzI+Onr7hY" + A(58) + "Ly+0mufnLb/9PP2/////v/ExOT/hITR/6mo6v+vruz/trTz/5CUx/+XmMb/9ff2/21uWf8qKnH/LDCb/3Z14v9mY+H/Wlff/1ZU2v9JRt3/hIXTzQ" + A(58) + "1tjkO56euf/z8/T/1NPx/6ak8/+sqPb/j5LT/7698//Pzfr/pqjS/2Vppf8ZF4z/CglS/62tyf9ZWqv/dnPo/2dl4P9bWN//VFLa/3Jx3v9RT9v/xcfgVg" + A(52) + "DMzd1Ora/Ehba513ZVWb3/i4vi/36ByP96f8D/w8H4/8TD8v/Fw/f/io/B/0JImf8LDpP/GyCI/3d32f93dej/ZmTf/1tY4v9OTsr/Y2PN/2Ng3v99fdfX" + A(53) + "OXn7h7k5u8hAAAAAJWZybFiZ8H/foHN/6+t8f+0svD/uLjx/7e28f+ysPP/lpjb/4WI0P+Pjun/hIHr/3Bv4f9oZOb/V1bT/1RT0f9STt//cHDV/1pZ1P/Z2+Yy" + A(64) + "6uzxFo6N4sCin/f/o6Hr/6qo7v+urO//rKru/6ak7P+hnvD/l5Xw/4eF6P97eeP/dHHp/15e0v9QUMr/WFXj/1NQ3/9jYdf/WFfS/6iszIw" + A(69) + "vsDeZ4B/4NqcmfD/n53s/6Gf6/+gnuv/m5nq/5KR6P+JiOb/goDn/3Jv6P9aWdP/TU/D/1JR0v9WVNr/VFHh/2FizP9UVsH/m57Hog" + A(74) + "wMPgZIaH28+Xlur/m5nv/5mX7v+Vk+3/j43t/4iG6f9zceDth4jXxqGlz5dvbtvxWFfV/1BQzP9KSNb/ZWfE/7y8y/+qq8qI" + A(80) + "oqbSm09SwP9wc8z/dHbR/3N00v9vcMz/VljH/4mNyMIAAAAAAAAAAO7w8Q7CxOVdnZ3hnoCB09TDxtha5uXsHbe4zm/z9PgE" + A(74) + "Df4PApUFDN/0tLz/9VUt3/TEvW/19gzf9QTdv/srTfeg" + A(36) + "Dy8vQH8PDzCw" + A(84) + "CLjdbEQUHM/05N2f9WU97/Z2bb/1xa2v/g4uoo" + A(133) + "NbY6Tt0eMXtiYu+/1layf9XVs//iIzBxg" + A(143) + "Ofp6Rrb2eA0tLPR/5mbvqXi5Ock" + A(149) + "NbY5DvGxtdW3N7nLw" + A(154) + "8vP2B+Xn7h0" + A(256) + "=";
...
async function gen_cfg(data, udp_en, tfo_en, mp, sp, rp, hp, tp, dns, secret, listmode) {
  ...
  if (listmode && /true/i.test(listmode)) {
    cfg = {};
